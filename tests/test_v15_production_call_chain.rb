...
def v15pc_edge(id:, start:, finish:, pid_path:)
  EdgeRecord.new(
    id:           id,
    source:       SourceReference.new(
      entity_id:            id,
      persistent_id:        id + 100,
      kind:                 'edge',
...
def v15pc_derived_edge(derived_id, occurrence_id:, start:, finish:)
...
test 'V15PC-003: WorkingModeRunner.run_duplicate_repair_batch runs proposer + executor from current registry' do
  ...
  records = [
    v15pc_derived_edge('der-edge-0-rec100', occurrence_id: 'occ-100',
                      start: e1.start_point, finish: e1.end_point),
    v15pc_derived_edge('der-edge-1-rec100', occurrence_id: 'occ-100',
...
test 'V15PC-004: no eligible actions -> ready + applied=0 (idempotent batch)' do
  ...
  records = [
    v15pc_derived_edge('der-A', occurrence_id: 'occ-100',
                      start: e1.start_point, finish: e1.end_point)
  ]
...
test 'V15PC-005: real-SketchUp constructible should-repair (same container path = CAD artifact)' do
  ...
  src = v15pc_snapshot(edges: [e1, e2])
  records = [
    v15pc_derived_edge('der-edge-0', occurrence_id: 'occ-100',
                      start: e1.start_point, finish: e1.end_point),
    v15pc_derived_edge('der-edge-1', occurrence_id: 'occ-100',
                      start: e2.start_point, finish: e2.end_point)
  ]
  ws = v15pc_workspace(snapshot: src, records: records)
  SUAnalysis::Core::WorkingModeRunner.instance_variable_set(:@current_workspace, ws)
...
test 'V15PC-006: shared-definition two instances are NOT merged (master plan §17.2)' do
  ...
  e1 = v15pc_edge(id: 0, start: [0.0, 0.0, 0], finish: [10.0, 0.0, 0],
                  pid_path: [100])  # instance A
  e2 = v15pc_edge(id: 1, start: [0.0, 0.0, 0], finish: [10.0, 0.0, 0],
                  pid_path: [200])  # instance B (same definition, different instance)
  src = v15pc_snapshot(edges: [e1, e2])
  records = [
    v15pc_derived_edge('der-A', occurrence_id: 'occ-100',
                      start: e1.start_point, finish: e1.end_point),
    v15pc_derived_edge('der-B', occurrence_id: 'occ-200',
                      start: e2.start_point, finish: e2.end_point)
  ]
  ws = v15pc_workspace(snapshot: src, records: records)
  SUAnalysis::Core::WorkingModeRunner.instance_variable_set(:@current_workspace, ws)
...
test 'V15PC-007: same-world-coords-different-provenance preserved' do
  ...
  src = v15pc_snapshot(edges: [e1, e2])
  records = [
    v15pc_derived_edge('der-A', occurrence_id: 'occ-100>200',
                      start: e1.start_point, finish: e1.end_point),
    v15pc_derived_edge('der-B', occurrence_id: 'occ-300>400',
                      start: e2.start_point, finish: e2.end_point)
  ]
  ws = v15pc_workspace(snapshot: src, records: records)
  SUAnalysis::Core::WorkingModeRunner.instance_variable_set(:@current_workspace, ws)
...
test 'V15PC-008: batch atomicity -- N-th action failure rolls back all prior actions' do
  ...
  records = [
    v15pc_derived_edge('der-A1', occurrence_id: 'occ-100',
                      start: e1.start_point, finish: e1.end_point),
    v15pc_derived_edge('der-A2', occurrence_id: 'occ-100',
                      start: e2.start_point, finish: e2.end_point),
    v15pc_derived_edge('der-B1', occurrence_id: 'occ-200',
                      start: e3.start_point, finish: e3.end_point),
    v15pc_derived_edge('der-B2', occurrence_id: 'occ-200',
...
test 'V15PC-009: Rebuild replay -> same post-state as first run' do
  ...
  records = [
    v15pc_derived_edge('der-A', occurrence_id: 'occ-100',
                      start: e1.start_point, finish: e1.end_point),
    v15pc_derived_edge('der-B', occurrence_id: 'occ-100',
                      start: e2.start_point, finish: e2.end_point)
  ]
  ws = v15pc_workspace(snapshot: src, records: records)
  SUAnalysis::Core::WorkingModeRunner.instance_variable_set(:@current_workspace, ws)
