...
# ---- H3: group-creation failure -> abort/fail closed ----
#
# V17-AIPM-DIRECT-SOURCE-REREVIEW-2026-09-01 RR-03 fix:
# the production V1.7 path is `workspace.build_entity` ONLY
# (no longer `adapter.add_line_to_repair_group`). The
# build_entity path goes through
# `adapter.create_top_level_group`; injecting failure there
# exercises the post-validate -> confirmed-abort path that
# must close the host operation truthfully (RR-01).
# Asserts: begin=1, abort=1, commit=0; workspace=:failed;
# zero surviving generated logical bridge records.

test 'V17-H3: group-creation failure -> abort/fail closed, no false APPLIED' do
  adapter = make_fake_adapter
  edges = [
    [[0.0, 0.0, 0.0], [10.0, 0.0, 0.0]],
    [[10.05, 0.0, 0.0], [20.0, 0.0, 0.0]]
  ...
  ws = make_test_workspace(adapter: adapter, edges: edges)
  # Inject a failure that breaks workspace.build_entity (the
  # current production bridge-creation path). build_entity
  # invokes @adapter.create_top_level_group; raising here
  # causes build_entity to return a :failed workspace,
  # which the executor then turns into a post-validate
  # failure and triggers the confirmed-abort path.
  adapter.define_singleton_method(:create_top_level_group) do |*_args, **_kw|
    raise StandardError, 'simulated_group_creation_failure'
  end
  good_proposal = [{
    'proposal_id' => 'p-fail',
    'state'       => GapPairProposer::STATE_READY_TO_REPAIR,
  ...
  assert_equal 0, commits, "no commit on group-creation failure"
  # RR-01: workspace MUST be :failed (not :ready / not :applied)
  # and must NOT advertise a successful APPLIED outcome.
  post_ws = result['post_workspace']
  assert_equal :failed, post_ws.state,
               "RR-01: post-validation failure -> workspace :failed; got #{post_ws.state.inspect}"
  # Source unchanged.
  refute_nil post_ws.source_snapshot
end

# ---- H4: commit uncertainty -> FAILED ----
#
# V17-AIPM-DIRECT-SOURCE-REREVIEW-2026-09-01 RR-01 fix:
# commit uncertainty must produce a :failed workspace AND
# must still CLOSE the operation truthfully. The stub
# below makes commit(commit: true) raise while leaving
# abort(commit: false) succeed -- the canonical
# `commit_uncertainty` outcome.
...
test 'V17-H4: commit uncertainty -> FAILED' do
  ...
  ws = make_test_workspace(adapter: adapter, edges: edges)
  # Stub end_operation: commit raises (commit_uncertainty),
  # abort succeeds AND properly logs the abort (operation
  # cleanly closed).
  original_end = adapter.method(:end_operation)
  adapter.define_singleton_method(:end_operation) do |m, commit:|
    if commit
      raise StandardError, "commit_uncertain_simulation"
    else
      # abort: delegate to the original end_operation so
      # the FakeAdapter's operation_log records the abort
      # truthfully.
      original_end.call(m, commit: commit)
    end
  end
  good_proposal = [{
    'proposal_id' => 'p-unc',
  ...
  # RR-01: even on commit uncertainty the operation MUST be
  # closed (no operation left open). The stub leaves the
  # FakeAdapter's @operation_open false after abort succeeds;
  # also assert it via the adapter's own state if exposed.
  op_log = adapter.operation_log
  begins  = op_log.select { |op| op[:kind] == :begin }.length
  commits = op_log.select { |op| op[:kind] == :commit }.length
  aborts  = op_log.select { |op| op[:kind] == :abort }.length
  assert_equal 1, begins, "H4: begin should fire exactly once"
  assert_equal 1, aborts, "H4: abort MUST close the operation after commit_uncertainty"
  assert_equal 0, commits, "H4: no commit ever happens on commit_uncertainty"
end
