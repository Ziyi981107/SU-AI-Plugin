...
### Lesson — Ruby Toolchain (2026-08-26, V1.5 Round-3 shell hang root cause)

- Host has two Ruby 2.7.8 executables:

  1. `C:\Ruby27-x64\bin\ruby.exe` — externally installed via RubyInstaller

     `.exe` self-extractor; **broken VC++ runtime / SxS configuration** on

     this host (ERROR_SXS_INVALID_DEACTIVATION). Fails identically under

     Git Bash (`Permission denied`), cmd.exe and PowerShell. Not an ACL

     issue, not a Git-Bash issue.

  2. `.vendor/ruby/rubyinstaller-2.7.8-1-x64/bin/ruby.exe` — bundled

     RubyInstaller-2.7.8-1 `.7z` extract; healthy, identical 34304-byte

     image, runs under any shell.

- **Preferred Ruby test shell for SU-AI-Plugin: PowerShell** (per project

  rule, August 2026). Bundled Ruby works there too.

- Targeted lookup ONLY: `Get-Command ruby -All`, `where.exe ruby`,

  `ruby --version`, direct existence checks. **Never use `find /` /

  `find C:\` / `find D:\` to locate Ruby** — a single such command has

  already caused two forced Agent terminations on this project.

- Environment failure is NOT evidence of a product-code regression.

  Always re-run targeted + full Ruby suite via the bundled executable

  after a shell anomaly before treating a test hang as a real defect.
...
**Round 3 worktree HEAD:** 7283a83 (in-progress; commit pending)



**Stable test state (pre-commit, after Round 3 code changes):**
  ...
  - Working tree: tracked changes (4 files) + 1 new file

    (derived_duplicate_topology.rb); see
...
**Round 3 fix summary:**
  ...
  - BLOCK-002 validator/proposer shared topology: new
    ...
    is the single source of truth for direct_match?, layer-0
  ...
  - BLOCK-001 + BLOCK-002 proposer fix: deduplicate_classes now

    partitions each connected component into MAXIMAL direct-

    match CLIQUES via Bron-Kerbosch-with-pivot. Each maximal

    clique is then verified by verify_final_merged_class_identity

    BEFORE any action is emitted. Verify-fail-closed results

    are routed through an `out_skipped` side-channel and

    converted to :skipped audit rows by build_actions. No
  ...
  - BLOCK-005: Pi recheck packet
    ...
    (`Review/V1_5_ROUND3_FIX_RECHECK_PACKET_2026-08-26.md`) is

    the truthful Round 3 record. Owner verification draft
...
**Stable commit:** pending (Round 3 commit will be recorded

  in CURRENT_STATE.md and in section 10 of the Round 3

  recheck packet upon completion).
